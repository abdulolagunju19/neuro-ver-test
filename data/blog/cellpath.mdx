...
a.	  
i.	(https://www.promega.ca/fr-ca/products/cell-signaling/gpcr-signaling/gtpase-glo-assay/?catNum=V7681)
b.	There are small molecules called GTPases that are very important for regulating cellular pathways. 
c.	The GTPases can be turned on or off. They are off when they are bound to a GDP (guanine diphosphate) and they are on when they are bound to a GTP (guanine triphosphate). Take a look at the picture below and try to figure which molecule is the GTP or GDP (hint: search up what a phosphate is).
d.	 
e.	A GAP will remove a phosphate from GTP, resulting in the GDP being attached to the GTPase.
f.	A GEF will replace the GDP on the GTPase with a GTP, activating the GTPase.
g.	GPCR: G Protein Coupled Receptor
h.	cAMP: cyclic AMP (adenosine monophosphate)
2.	G Protein:
